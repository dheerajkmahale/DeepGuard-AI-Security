...
const Dashboard = () => {
  ...
  const handleViewReport = (id: string) => {
    toast.info(`Viewing report for analysis ID: ${id}`, {
      description: "This is a placeholder. In a real application, a detailed report would be displayed here.",
      duration: 3000,
    });
  };

  const handleExportData = () => {
    const headers = ["ID", "File Name", "File Type", "Date", "Is Deepfake", "Confidence Score", "Threat Level"];
    const rows = analysisHistory.map(record => [
      record.id,
      record.fileName,
      record.fileType,
      record.date,
      record.isDeepfake ? "Yes" : "No",
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", "deepguard_analysis_data.csv");
  ...
  const stats = {
    ...
    avgConfidence: (analysisHistory.length > 0 ? analysisHistory.reduce((acc, a) => acc + a.confidenceScore, 0) / analysisHistory.length : 0).toFixed(1)
  };
        ...
        <div className="container max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4">
              ...
              <Button
                variant={filter === 'deepfake' ? 'default' : 'ghost'}
                size="sm"
              ...
              <Button
                variant={filter === 'authentic' ? 'default' : 'ghost'}
                size="sm"
          ...
          <Card className="p-6 bg-card/50 backdrop-blur-sm border-border">
            ...
            <div className="space-y-4">
              {analysisHistory
                .filter(record => {
                  if (filter === 'deepfake') return record.isDeepfake;
                  if (filter === 'authentic') return !record.isDeepfake;
                  return true;
                })
                .map((record) => (
                <div
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleViewReport(record.id)}>
